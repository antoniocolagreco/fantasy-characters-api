...
- [🧪 Testing Guide](docs/testing.md) - Testing strategy, best practices, and
  templates
- [🔥 Smoke Testing](docs/smoke-testing.md) - Deployment validation and health
  checks
...
- **Email Verification Gate** - Verification flow with tokenized links and RBAC
  enforcement blocking CREATE for unverified users (configurable via
  `EMAIL_VERIFICATION_ENABLED`)
- **SMTP Email (Optional)** - Outbound email delivery using Nodemailer over
  SMTP for verification emails (no-op when SMTP is not configured)
- **Argon2 Password Hashing** - Modern password hashing for secure credential
  storage
