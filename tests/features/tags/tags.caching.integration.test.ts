...
describe('Tags API - Caching', () => {
    ...
    it('returns Cache-Control and ETag for list; supports 304 with If-None-Match', async () => {
        ...
        await createTestTag({ name: 'Tag A' })
        await createTestTag({ name: 'Tag B' })

        const first = await app.inject({
            method: 'GET',
            url: '/api/v1/tags?limit=2',
            headers: {
    ...
    it('returns Cache-Control and ETag for getById; supports 304 with If-None-Match', async () => {
        await cleanupTestData()
        const tag = await createTestTag({ name: 'Unique Tag' })
