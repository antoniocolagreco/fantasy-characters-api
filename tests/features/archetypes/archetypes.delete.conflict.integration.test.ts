...
async function createAdmin() {
    return prisma.user.create({
        data: {
            id: generateUUIDv7(),
            email: `admin-arch-${Date.now()}@test.dev`,
            passwordHash: 'x',
            role: 'ADMIN',
...
describe('Archetype deletion conflict (409)', () => {
    ...
    it('returns 409 when archetype is referenced by character', async () => {
        const admin = await createAdmin()
        const race = await createRace(admin.id)
        const archetype = await createArchetype(admin.id)
        await createCharacter(admin.id, race.id, archetype.id)
        const headers = createAuthHeaders({ id: admin.id, role: 'ADMIN', email: admin.email })

        const res = await app.inject({
            method: 'DELETE',
            url: `/api/v1/archetypes/${archetype.id}`,
            headers,
        })
    ...
    it('deletes archetype when unused', async () => {
        const admin = await createAdmin()
        const archetype = await createArchetype(admin.id)
        const headers = createAuthHeaders({ id: admin.id, role: 'ADMIN', email: admin.email })
        const res = await app.inject({
