...
describe('Environment Configuration', () => {
    ...
    it('should throw error for invalid configuration', () => {
        vi.stubEnv('NODE_ENV', 'invalid-env') // Invalid NODE_ENV
        vi.stubEnv('PORT', 'not-a-number') // Invalid PORT
        vi.stubEnv('DATABASE_URL', '') // Invalid DATABASE_URL (too short)

        expect(() => loadConfig()).toThrow('Invalid configuration')
    })

    it('should handle MAX_FILE_SIZE parsing', () => {
        vi.stubEnv('NODE_ENV', 'test')
        vi.stubEnv('MAX_FILE_SIZE', '1048576') // 1MB in bytes
